...
| 中控总览 | 集中展示 OV5640、MPU6050、压力传感器、DFPlayer、RGB、泵/阀状态、实时数据、推导与执行结果；串口/模拟接入移至顶层设备入口 | 固定画面模式默认开启，避免高频刷新造成卡顿；各卡片只陈述当前可获得的链路数据 |
...
| 串口遥测 | 解析并记录 pressure、motion v2、camera_status、action_status、pneumatic_status | MPU6050 已有 COM21 基础读数证据；压力/气动最终硬件仍未验收 |

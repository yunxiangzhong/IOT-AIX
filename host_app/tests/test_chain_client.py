...
class ChainClientTests(unittest.TestCase):
    def test_normalizes_service_url_and_extracts_frame_identity(self):
        ...
        self.assertEqual(
            frame_identity_from_state({"boot_id": "0123456789abcdef", "upload": {"last_frame_seq": 17}}),
            ("0123456789abcdef", 17),
        )

    def test_rejects_invalid_state_identity(self):
        self.assertIsNone(frame_identity_from_state({"boot_id": "", "upload": {"last_frame_seq": -1}}))

...
class CooperativeScenarioPanel(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        detail = QtWidgets.QLabel("路侧摄像头发现右侧盲区货车，云端预测后在到达路口前通知头盔 ESP32")
    ...
    def _update_from_elapsed(self, elapsed_ms: int) -> None:
        ...
        if elapsed_ms < 260:
            ...
            if not self._dispatched:
                ...
                self.stages[3].set_state("正在下发真实头盔链路", "active")
                self.helmet_status[1].setText("预警已送入上位机服务，等待串口 ACK")
                self.start_requested.emit(self.demo_payload(remaining_ms))

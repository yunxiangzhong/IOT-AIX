...
| MPU6050 | I2C 驱动、motion v2 串口事件、运动检测与 C 测试已实现 | COM21 已测得约 99.69 Hz、无读错；仍未完成头盔佩戴状态的静置校准、冲击阈值和现场误报率验证 |
| 气泵 / 三通电磁阀 | GPIO40、GPIO41、手动脉冲、泄压、急停、故障锁存与策略测试已实现 | COM21 串口已回传高风险下泵开、阀通电和有效压力；没有独立泵/阀电流或位置传感器 |
| 自动充气条件 | 有效视觉 HIGH/CRITICAL 回调直接充气；MPU impact/rapid_tilt 是独立备用触发，不是视觉高风险的前置条件 | 当前 COM21 运行配置已明确开启；上位机不能修改 ESP32 的保护限制 |
...
COM21 本次验收采用 `sdkconfig.runtime` 的控制/自动模式均为 `y`：新鲜有效的视觉 HIGH/CRITICAL 立即输出充气，不等待 MPU；持续风险不会使用通用 2 秒计时进入 `inflate_timeout`。达到保存的 `8 kPa` 目标、视觉失效、压力无效/过期、超过 `200 kPa` 硬上限或急停会关闭泵阀泄压。

...
| 上位机中控总览 | Apple 浅色单页界面、顶层设备接入、诊断/标定入口、稳定画面模式和状态趋势已实现；18 张映射卡采用唯一权威数据源和内容去重，主机侧 82 项测试通过 | 默认显示固定已分析画面；状态卡不会被健康心跳、普通采样和串口诊断轮流覆盖，但这些软件约束不等于实机安全验收 |
...
生产上位机已删除路口动画、固定货车 ETA、虚拟骑行者减速和主动提交演示事件的入口。正式链路只接受外部真实系统提交且 `simulated=false` 的事件：

1. 外部路侧系统向 PC 服务提交真实事件。
2. PC 服务校验事件并转发给当前设备。
3. ESP32 校验设备、boot、TTL 和事件内容，返回真实 ACK 并仲裁 RGB。
4. 上位机只记录 PC 链路状态和 ESP32 串口反馈，不用 ACK 推断语音或气动执行结果。

真实路侧相机、云端识别和现场 ETA 尚未联调，因此当前只验证了协议与固件策略，不能宣称已完成真实交通监控。
